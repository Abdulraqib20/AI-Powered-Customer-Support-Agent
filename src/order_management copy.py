    ...
    def place_order(self, customer_id, cart_items, delivery_address, payment_method, notes=""):
        ...
        try:
            ...
            if result.get('success'):
                ...
                # 🔧 FIX: Extract order_summary instead of order_details
                order_summary = result.get('order_summary')
                logger.info(f"✅ Order {order_id} placed successfully for customer {customer_id}")

                # Convert OrderSummary dataclass to dict for proper formatting
                if hasattr(order_summary, '__dict__'):
                    ...
                    order_details_dict = {
                        ...
                        'estimated_delivery': order_summary.estimated_delivery,
                        'items': [],
                        'order_items': []
                    }
