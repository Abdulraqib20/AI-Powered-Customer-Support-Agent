...
class OrderAIAssistant:
    ...
    def __init__(self, memory_system: Optional[WorldClassMemorySystem]):
        ...
        # 🔧 CRITICAL FIX: Initialize database configuration
        try:
    ...
    def extract_product_info(self, user_message: str, product_name_hint: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # 🔧 ENHANCED SEARCH: Multi-strategy search with abbreviation support
            with self.get_database_connection() as conn:
                with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                    ...
                    # Strategy 2: Enhanced abbreviation and fuzzy matching
                    # Create common abbreviation mappings
                    ...
                    for word in cleaned_words:
                        if word in abbreviation_mappings:
                            expanded_terms.append(abbreviation_mappings[word])
                            print_log(f"🔄 Expanded '{word}' to '{abbreviation_mappings[word]}'")
                        else:
                            expanded_terms.append(word)

                    # Search with expanded terms
                    if expanded_terms != cleaned_words:
                        ...
                        cursor.execute("""
                            ...
                            ORDER BY
                                ...
                                product_name
                            LIMIT 1
                        """, (f"%{expanded_search}%", expanded_search))

                        product = cursor.fetchone()
                        if product:
                            print_log(f"✅ Product found via Abbreviation expansion: {product['product_name']}", 'success')
                            product_dict = dict(product)
                            self._last_mentioned_product = product_dict
                            logger.info(f"🎯 EXTRACTED PRODUCT FOR CONTEXT: {product_dict['product_name']} (ID: {product_dict['product_id']})")
                            return product_dict

                    # Strategy 3: Individual word search within the same product
                    ...
                    # Strategy 4: Partial match with minimum 3 characters
                    if len(cleaned_product_name) >= 3:
                        cursor.execute("""
                            SELECT product_id, product_name, category, brand, description, price, currency, in_stock, stock_quantity,
                                   CASE
                                       WHEN LOWER(product_name) LIKE LOWER(%s) THEN 1
                                       WHEN LOWER(product_name) LIKE LOWER(%s) THEN 2
                                       ELSE 3
                                   END as match_priority
                            FROM products
                            WHERE (LOWER(product_name) LIKE LOWER(%s) OR LOWER(product_name) LIKE LOWER(%s))
                              AND in_stock = TRUE
                            ORDER BY match_priority, product_name
                            LIMIT 1
                        """, (
                            f"{cleaned_product_name}%",  # Starts with
                            f"%{cleaned_product_name}%", # Contains
                            f"{cleaned_product_name}%",  # Starts with
                            f"%{cleaned_product_name}%"  # Contains
                        ))
                        ...
                        product = cursor.fetchone()
                        if product:
                            print_log(f"✅ Product found via Partial matching: {product['product_name']}", 'success')
                            product_dict = dict(product)
                            self._last_mentioned_product = product_dict
                            logger.info(f"🎯 EXTRACTED PRODUCT FOR CONTEXT: {product_dict['product_name']} (ID: {product_dict['product_id']})")
                            return product_dict
